...
3) Reparar y normalizar relaciones de Inventario (evitar AUTO-*)
- Ejecutar el script de reparación para poblar FKs reales en `dbo.Inventario` (cliente/producto/estación/usuario) y normalizar nombres de catálogos:
  - Desde SSMS: abrir `scripts/repair_inventory_joins.sql` y ejecutar sobre `SOConteo`.
  - Con `sqlcmd` (opcional):
    `sqlcmd -S <SERVIDOR> -d SOConteo -E -i scripts\repair_inventory_joins.sql`
...
5) Forzar uso de SQL Server en el backend
